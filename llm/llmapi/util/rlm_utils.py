...
def add_execution_result_to_messages(messages: List[Dict[str, str]], 
        code: str, 
        result: str,
        max_character_length: int = 1000000,
    ) -> List[Dict[str, str]]:
    """
...
def execute_code(repl_env, code: str) -> str:
    """
    ...
    try:
        ...
        logger.info(f"code: {code}, CODE_EXECUTION: {formatted_result}")
        return formatted_result
        
    except Exception as e:
        error_msg = f"Error executing code: {str(e)}"
        logger.error(f"code: {code}, {error_msg}")
        return error_msg
...
def check_for_final_answer(response: str, repl_env) -> Optional[str]:
    ...
    if answer_type == 'FINAL':
        ...
        try:
            ...
            if variable_name in repl_env.locals:
                ...
                logger.info("FINAL_VAR: " + error_msg)
                return None
        except Exception as e:
            error_msg = f"Error retrieving variable '{variable_name}': {str(e)}"
            logger.error(f"FINAL_VAR: {error_msg}")
            return None
