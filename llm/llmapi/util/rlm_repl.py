...
from util.rlm import RLM
from util.rlm_prompts import DEFAULT_QUERY, next_action_prompt, build_system_prompt
from util.mylog import logger
import util.rlm_utils as utils
...
class Sub_RLM(RLM):
    ...
    def completion(self, prompt) -> str:
        try:
            ...
            isok, response = self.client.chat(
                prompt=prompt,
            )
            if not isok:
                return f"Error making LLM query: {response}"
...
class REPLEnv:
    def __init__(
        ...
        self.sub_rlm: RLM = Sub_RLM(llm_client=llm_client)
    ...
    def load_context(self, context_json: Optional[dict | list] = None, context_str: Optional[str] = None):
        ...
        if context_json is not None:
            ...
            logger.info(f"Context JSON saved to => {context_path}")
            context_code = (
                f"import json\n"
        ...
        if context_str is not None:
            ...
            with open(context_path, "w") as f:
                f.write(context_str)
            logger.info(f"Context text saved to => {context_path}")
            context_code = (
                f"import os\n"
                f"with open(r'{context_path}', 'r') as f:\n"
                f"    context = f.read()\n"
    ...
    def code_execution(self, code) -> REPLResult:
        ...
        logger.info(f"Execution time: {execution_time:.4f} seconds, stdout: {stdout_content}, stderr: {stderr_content}")
...
class RLM_REPL(RLM):
    ...
    def __init__(self, 
            ...
            recursive_model: Optional[Any] = None,
            max_iterations: int = 20,
            depth: int = 0,
        ):
        self.llm = llm_client # Replace with other client
        self.recursive_model = recursive_model or llm_client
    ...
    def setup_context(self, context: List[str] | str | List[Dict[str, str]], query: Optional[str] = None):
        ...
        self.repl_env = REPLEnv(
            llm_client=self.recursive_model,
    ...
    def completion(self, context: List[str] | str | List[Dict[str, str]], query: Optional[str] = None) -> str:
        ...
        logger.info(f"RLM_REPL: Starting new REPL session with completion: {len(self.query)}, messages: {len(self.messages)}")
        
        # Main loop runs for fixed # of root LM iterations
        for iteration in range(self._max_iterations):
            logger.info(f"RLM_REPL: Iteration {iteration} starting.")
            # Query root LM to interact with REPL environment
            prompt = next_action_prompt(query, iteration)
            isok, response = self.llm.chat(prompt=prompt, messages=self.messages)
            if not isok:
                logger.error(f"RLM_REPL: Iteration {iteration}, LLM response failed: {response}")
                raise Exception(f"RLM_REPL: LLM query failed: {response}")
            
            logger.info(f"RLM_REPL: Iteration {iteration}, LLM response: {response}")
            ...
            final_answer = utils.check_for_final_answer(
                response, self.repl_env,
            )

            # In practice, you may need some guardrails here.
            if final_answer:
                logger.info(f"RLM_REPL: Final answer found in iteration {iteration}: {final_answer}")
                return final_answer

        # If we reach here, no final answer was found in any iteration
        logger.info(f"RLM_REPL: No final answer found in any iteration after {self._max_iterations} iterations.")
        prompt = next_action_prompt(query, iteration, final_answer=True)
        isok, final_answer = self.llm.chat(prompt=prompt, messages=self.messages)
        if not isok:
            logger.error(f"RLM_REPL: Final answer LLM response failed: {final_answer}")
            raise Exception(f"RLM_REPL: Final answer LLM query failed: {final_answer}")
        
        logger.info(f"RLM_REPL: Final answer found after max iterations: {final_answer}")
